...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  %Backpropagation
  output_diff = class_prob - data.targets;
  hidden_diff = model.hid_to_class' * output_diff .* hid_output .* ( 1 - hid_output );
  
  output_delta = output_diff * hid_output';
  hidden_delta = hidden_diff * data.inputs';
  ...
  ret.input_to_hid = 1 / m * hidden_delta + model.input_to_hid * wd_coefficient;
  ret.hid_to_class = 1 / m * output_delta + model.hid_to_class * wd_coefficient;
end
